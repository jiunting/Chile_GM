...
$run_num='024513'; #so that the path of rupt file=rupt_file+'/'+'subduction.'+run_num+'.rupt'
#$run_num='026898'; #so that the path of rupt file=rupt_file+'/'+'subduction.'+run_num+'.rupt'
...
#`gmt grdimage $grdfile -I$grdgrad -JL$area_L -R$area -Ctopo.cpt -Y2i -X0.7i -P -t70  -K > Chile_rupts_base.ps`;
#
...
#    `awk '{print \$5,\$6}' slab.tmp >> element.xy`;
#    `gmt psxy element.xy -R -J -L -Cslip.cpt -O -K >>Chile_rupts_base.ps`; #Use GMT5 if this has issue (plot the triangle color)
#    #`gmt psxy element.xy -R -J -W0.01p,100/100/100 -O -K >> $fileout`;
#    `gmt psxy element.xy -R -J -W0.01p,200/200/200 -O -K >> Chile_rupts_base.ps`; #Plot triangle boundary
#    if ($a1==50000){
#        last;
#    }
#}
...
#`gmt psscale -Dx0.5i/1.8i/1.5i/0.15i -Ba10f5:"depth(km)": -Cdepth.cpt -E  -O -K >> $fileout".ps"`;
    `gmt gmtset MAP_ANNOT_OFFSET_PRIMARY 0p MAP_LABEL_OFFSET 0.5p FONT_LABEL 14p FONT_ANNOT_PRIMARY 12p`;
    `gmt psscale -Dx0.4i/3.2i/0.8i/0.12i -Ba15f7.5:"slip(m)": -Cslip.cpt -E  -O -K >> $fileout".ps"`;
